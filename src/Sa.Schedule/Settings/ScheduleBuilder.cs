...
internal class ScheduleBuilder : IScheduleBuilder
{
    ...
    public IJobBuilder AddJob<T>(Guid? jobId = null)
        where T : class, IJob
    {
        Guid id = GetId(jobId);
    ...
    public IScheduleBuilder AddJob<T>(Action<IServiceProvider, IJobBuilder> configure, Guid? jobId = null)
        where T : class, IJob
    {
        Guid id = GetId(jobId);
        _services.TryAddKeyedScoped<T>(id);
